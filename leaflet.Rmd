...
Now, let's construct the body adding the map object. The view can be set using `var map = L.map('map', {center: [20, -15], zoom: 3});` and just like in in R Studio, different [basemaps](https://leaflet-extras.github.io/leaflet-providers/preview/) can be used wiht the line `L.tileLayer('http://{s}.tiles.wmflabs.org/bw-mapnik/{z}/{x}/{y}.png', {attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'}).addTo(map);`. 
